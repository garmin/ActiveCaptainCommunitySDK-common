...
/**
    @file
    @brief File system utility functions - POSIX implementation
...
bool Delete(const std::string& aFilePath) {
  int rcode = unlink(aFilePath.c_str());
...
bool Exists(const std::string& aFilePath) {
  return access(aFilePath.c_str(), 0) == 0;
}  // end of Exists
...
bool GetSize(const std::string& aFilePath, uint64_t& aFileSize) {
  ...
  int rcode = stat(aFilePath.c_str(), &stats);
